...
RobotController::RobotController(std::shared_ptr<rclcpp::Node> node, QObject* parent)
    : QObject(parent),
      node_(node),
      current_drive_mode_(DriveMode::FullDrive),
      current_pairs_grouping_mode_(PairsGroupingMode::AllPairs),
      controllers_({{{WheelsControllerName::FrontLeft,
  ...
  qmlRegisterUncreatableType<WheelJointName>("WheelJointName", 1, 0, "WheelJointName",
                                             "Not creatable as it is an enum type.");
  qmlRegisterUncreatableType<WheelsControllerName>("WheelsControllerName", 1, 0, "WheelsControllerName",
                                                   "Not creatable as it is an enum type.");
...
void RobotController::setPairsGroupingMode(PairsGroupingMode::Mode mode) {
...
void RobotController::setWheelsControllerState(WheelsControllerName::Name controller_name,
                                               WheelsControllerState::State state) {
  for (Controller& controller : controllers_) {
    if (controller.wheels_controller_name == controller_name) {
      controller.wheels_controller_state = state;
...
void RobotController::createROSInterfaces() {
  ...
  for (auto controller_enum : WheelsControllerName::getAllNames()) {
    std::string controller_name = WheelsControllerName::toString(controller_enum);

    auto publisher =
        node_->create_publisher<std_msgs::msg::Float64MultiArray>("/" + controller_name + "/commands", 100);
  ...
  joint_state_subscriber_ = node_->create_subscription<sensor_msgs::msg::JointState>(
      "/joint_states", 100, std::bind(&RobotController::jointStatesCallback, this, std::placeholders::_1));
}
...
void RobotController::jointStatesCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  ...
  for (size_t i = 0; i < msg->name.size(); i++) {
    ...
    WheelJointName::Name wheel_joint_enum = WheelJointName::fromString(joint_name);
...
QVariantList RobotController::controllers() const
{
  ...
  for (const Controller& controller : controllers_) {
    ...
    controllerMap["name"] = controller.wheels_controller_name;
    controllerMap["state"] = controller.wheels_controller_state;
    controllerMap["outerJoint"] = QVariantMap{{"name", controller.outer_joint.name},
    ...
    controllerMap["innerJoint"] = QVariantMap{{"name", controller.inner_joint.name},
    ...
    controllerMap["rayJoint"] = QVariantMap{{"name", controller.ray_joint.name},
...
void RobotController::publishSpeed(rclcpp::Publisher<std_msgs::msg::Float64MultiArray>::SharedPtr publisher,
...
void RobotController::publishGlobalSpeed(double speed)
{
  ...
  for (const auto& controller : controllers_) {
    if (controller.wheels_controller_state == WheelsControllerState::Global &&
        pair_velocity_publishers_.find(controller.wheels_controller_name) != pair_velocity_publishers_.end()) {
      publishSpeed(pair_velocity_publishers_[controller.wheels_controller_name], msg);
    }
  }
}

void RobotController::publishLocalSpeed(double speed, WheelsControllerName::Name controller_name) {
  std_msgs::msg::Float64MultiArray msg;
  double speed_in_radians = qDegreesToRadians(speed);
  msg.data = {speed_in_radians, speed_in_radians};
  ...
  for (Controller& controller : controllers_) {
    if (controller.wheels_controller_name == controller_name &&
        controller.wheels_controller_state == WheelsControllerState::Local) {
      if (pair_velocity_publishers_.find(controller_name) != pair_velocity_publishers_.end()) {
        publishSpeed(pair_velocity_publishers_[controller_name], msg);
      }
      break;
    }
  }
...
void RobotController::publishIndependentSpeed(double speed, WheelsControllerName::Name controller_name,
                                              bool is_outer_joint) {
  std_msgs::msg::Float64MultiArray msg;
  double speed_in_radians = qDegreesToRadians(speed);
  ...
  for (Controller& controller : controllers_) {
    if (controller.wheels_controller_name == controller_name &&
        controller.wheels_controller_state == WheelsControllerState::Independent) {
...
void RobotController::publishRayPosition(double position, RayJointName::Name ray_name) {
  ...
  if (ray_position_publishers_.find(ray_name) != ray_position_publishers_.end()) {
    auto publisher = ray_position_publishers_[ray_name];
    std::string topic_name = publisher->get_topic_name();

    if (publisher->get_subscription_count() > 0) {
      publisher->publish(msg);
      QString logMsg =
          QString("Published ray position %1 m to %2").arg(position, 0, 'f', 3).arg(QString::fromStdString(topic_name));
      addLogMessage(logMsg);
    } else {
